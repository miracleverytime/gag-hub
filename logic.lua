...
return function(ctx)
    ...
    RunService.Heartbeat:Connect(function()
        ...
        if States.espMutations then
            -- Game asli pakai: workspace:QueryDescendants("[$Mutation]")
            -- artinya scan SEMUA object di workspace yang punya attribute "Mutation"
            -- tidak terbatas pada folder Plants milik sendiri saja.
            -- Kita tiru dengan GetDescendants + filter attribute.
            for _, obj in ipairs(workspace:GetDescendants()) do
                -- Game hanya apply mutation ke Model atau Tool (dari decompiled source)
                if not (obj:IsA("Model") or obj:IsA("Tool")) then continue end
                local mut = obj:GetAttribute("Mutation")
                if not mut or mut == "" or mut == "None" then continue end

                local rootPart = GetModelRootPart(obj)
                if not rootPart then continue end
                if rootPart:FindFirstChild("MiracleESP_Mut") then continue end

                -- SeedName: coba beberapa attribute name yang mungkin dipakai game
                local sn = obj:GetAttribute("SeedName")
                    or obj:GetAttribute("SeedTool")
                    or obj:GetAttribute("Name")
                    or obj.Name

                -- Guard: kalau nama terlihat seperti instance ID acak (panjang & non-alphabetic)
                if sn and (#sn > 24 or sn:match("^%d")) then
                    sn = "Plant"
                end
