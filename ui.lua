...
return function(ctx)
    ...
    registerPage("Profile", function()
        ...
        local activeVal   = statCell(4, "\226\136\191", "0",        "ACTIVE")

        -- ── Human-readable labels for each automation key ──
        local KEY_LABELS = {
            ...
            autoBuyAll         = "Auto Buy All",
            autoBuyGear        = "Auto Buy Gear",
            autoBuyGearAll     = "Auto Buy Gear All",
            autoBuyCrate       = "Auto Buy Crate",
            autoBuyCrateAll    = "Auto Buy Crate All",
            autoOpenCrate      = "Auto Open Crate",
            autoSell           = "Auto Sell",
            autoCatchWild      = "Auto Catch Wild",
            fly                = "Fly",
        ...
        -- ── Active-loops popup (anchored to the ACTIVE cell) ──
        -- Find the ACTIVE cell (parent of activeVal)
        local activeCell = activeVal.Parent

        -- Small "list" icon button, top-right corner of the ACTIVE cell
        local loopBtn = Create("TextButton", {
            Parent = activeCell,
            Size = UDim2.new(0, 18, 0, 18),
            Position = UDim2.new(1, -26, 0, 8),
            BackgroundColor3 = Colors.Surface,
            BorderSizePixel = 0,
            Text = "\226\137\\186", -- ≺ list glyph substitute; we'll use ☰ (trigram)
            TextColor3 = Colors.TextMuted,
            TextSize = 11,
            Font = FONT_MONO,
            ZIndex = 10,
        })
        -- Override with a proper list/eye symbol
        loopBtn.Text = "\226\152\175" -- ☯ fallback; actual char below
        loopBtn.Text = utf8.char(0x2630) -- ☰  trigram / "list" icon
        CreateCorner(loopBtn, 4)

        -- Hover tint
        loopBtn.MouseEnter:Connect(function()
            Tween(loopBtn, {TextColor3 = Colors.Accent}, 0.15)
        end)
        loopBtn.MouseLeave:Connect(function()
            Tween(loopBtn, {TextColor3 = Colors.TextMuted}, 0.15)
        end)

        -- ── Popup overlay (parented to ctx.MainFrame so it floats above everything) ──
        local loopPopupOpen = false
        local loopPopup     -- forward decl

        local function buildLoopPopup()
            -- Collect currently-active keys
            local activeList = {}
            for _, k in ipairs(AUTOMATION_KEYS) do
                if States[k] then
                    activeList[#activeList + 1] = KEY_LABELS[k] or k
                end
            end

            -- Destroy old popup if any
            if loopPopup and loopPopup.Parent then
                loopPopup:Destroy()
            end

            -- Popup frame — floats over the profile content
            loopPopup = Create("Frame", {
                Parent = ctx.MainFrame,
                Size = UDim2.new(0, 220, 0, 0),   -- height set below
                BackgroundColor3 = Colors.BackgroundLighter,
                BorderSizePixel = 0,
                ZIndex = 50,
                AutomaticSize = Enum.AutomaticSize.Y,
            })
            CreateCorner(loopPopup, 10)
            CreateStroke(loopPopup, Colors.BorderLight, 1)

            -- Position: anchor relative to activeCell inside MainFrame
            -- We derive absolute position at runtime
            task.defer(function()
                if not loopPopup.Parent then return end
                if not activeCell.Parent then return end
                local cellAbs  = activeCell.AbsolutePosition
                local mainAbs  = ctx.MainFrame.AbsolutePosition
                local relX     = cellAbs.X - mainAbs.X
                local relY     = cellAbs.Y - mainAbs.Y + activeCell.AbsoluteSize.Y + 6
                -- Clamp so popup doesn't overflow right edge
                local mainW    = ctx.MainFrame.AbsoluteSize.X
                if relX + 220 > mainW - 8 then
                    relX = mainW - 228
                end
                loopPopup.Position = UDim2.new(0, relX, 0, relY)
            end)
            ...
            CreatePadding(inner, 10)
            CreateListLayout(inner, 4)
            ...
            if #activeList == 0 then
                ...
                for i, label in ipairs(activeList) do
                    local row = Create("Frame", {
                        Parent = inner,
                        Size = UDim2.new(1, 0, 0, 26),
                        BackgroundTransparency = 1,
                    ...
                    Create("Frame", {
                        ...
                        BorderSizePixel = 0,
                        ZIndex = 52,
                    }):FindFirstChildWhichIsA("UICorner") or CreateCorner(row:FindFirstChildWhichIsA("Frame") or row:WaitForChild("Frame"), 3)
                    -- Dot corner
                    do
                        local dot = row:FindFirstChildOfClass("Frame")
                        if dot then CreateCorner(dot, 3) end
                    end
                    Create("TextLabel", {
                        Parent = row,
        ...
        task.spawn(function()
            while sessionVal.Parent do
                ...
                -- Refresh popup content live if it's open
                if loopPopupOpen and loopPopup and loopPopup.Parent then
                    buildLoopPopup()
                end
                task.wait(1)
            end
